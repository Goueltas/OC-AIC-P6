...
def action(choix):
    ...
    if choix == "1":
        nom_pc = raw_input("Indiquez le nouveau nom de ce PC :")
        chname.change_hostname(nom_pc)
    elif choix == "2":
        new_user = raw_input("Indiquez le nom de l'utilisateur ou du fichier des utilisateurs à créer :")
        ...
        old_user = raw_input("Indiquez le nom de l'utilisateur ou du fichier des utilisateurs à supprimer :")
...
def continuer():
    ...
    """
    reponse = raw_input("Voulez-vous effectuer une autre opération ? [o/N]")
    if reponse.upper() == "O":
        return True
    else:
        return False
